...
public class PathUtils {
   ...
   * @return An array of File objects representing the files in the specified directory.
   *         or empty array if the directory is null or if there are no files.
   */
  public static File[] getFiles(File directory) {
    return Optional.ofNullable(directory).map(File::listFiles).orElse(new File[0]);
  }
